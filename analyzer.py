...
def format_analysis(res: dict) -> str:
    """analyze 결과를 콘솔·리포트 공용 형식으로 만든다."""
    out = ["=== AI 인사이트 분석 결과 ==="]
    ...
    section("주요 트렌드", res.get("trends"))
    if res.get("keywords"):
        out.append("[핵심 키워드]")
        out.append(", ".join(map(str, res["keywords"])))
    section("공통점", res.get("common_points"))
    section("차이점", res.get("differences"))
    section("주요 이슈", res.get("issues"))
    section("시사점", res.get("implications"))
    return "\n".join(out)
